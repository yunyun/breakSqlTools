...
# 备份文件存储目录
backup_dir = "/work/break/sql"
date = datetime.now().strftime("%Y%m%d")
backup_dir_full_path = os.path.join(backup_dir, date)

# 创建备份目录
os.makedirs(backup_dir_full_path, exist_ok=True)
...
mysql_configs = [
    {
        "host": "127.0.0.1",
        "user": "yunyun",
        "password": "123456",  # getpass("Enter MySQL password for ddfs1: "),  # 出于安全考虑，这里使用 getpass 获取密码
        "port": 3306,
        "databases": ["test1"]  # 使用列表来存储多个数据库
    },
    {
        "host": "127.0.0.1",
        "user": "yunyun",
